...
async def submit_task(request: TaskSubmitRequest):
    ...
    if request.task_type not in valid_types:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid task type: {request.task_type}. Valid values: {valid_types}"
        )
...
async def cancel_task(task_id: str):
    ...
    if task_info.status in (TaskStatus.COMPLETED, TaskStatus.FAILED, TaskStatus.CANCELLED):
        raise HTTPException(
            status_code=400,
            detail=f"Task cannot be cancelled: status is {task_info.status}"
        )
...
async def submit_fund_fetch_task(request: FundFetchRequest):
    ...
    return TaskSubmitResponse(
        task_id=task_id,
        status="pending",
        message="Fund fetch task submitted"
    )
...
async def submit_score_calculation_task(request: ScoreCalculationRequest):
    ...
    return TaskSubmitResponse(
        task_id=task_id,
        status="pending",
        message="Score calculation task submitted"
    )
...
async def submit_cache_warmup_task(request: CacheWarmupRequest):
    ...
    return TaskSubmitResponse(
        task_id=task_id,
        status="pending",
        message="Cache warmup task submitted"
    )
