...
class Watchlist(Base, UserMixin):
    ...
    __table_args__ = (Index("idx_watchlist_user_code", "user_id", "code", unique=True),)
...
def create_async_engine_from_url(url: str, echo: bool = False, pool_size: int = 5, max_overflow: int = 10, **kwargs):
    """创建异步引擎（PostgreSQL 使用 postgresql+asyncpg://）"""
    global _async_engine, _async_session_factory

    _async_engine = create_async_engine(url, echo=echo, pool_size=pool_size, max_overflow=max_overflow, **kwargs)
