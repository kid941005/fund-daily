...
class CacheManager:
    ...
    def __init__(self):
        self._stats = {
            'hits': 0,
            'misses': 0,
            'penetration_protected': 0,
            'avalanche_protected': 0,
            'errors': 0
        }
...
def cached(
    ttl: int = 300,
    key_prefix: str = "",
    use_penetration_protection: bool = True
):
    """
    缓存装饰器
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if use_penetration_protection:
                ...
                return manager.get_with_penetration_protection(
                    key, loader, ttl=ttl, empty_ttl=60
                )
            else:
                # 普通缓存
