...
def fund_not_found(fund_code: str) -> FundServiceError:
    return FundServiceError(ErrorCode.FUND_NOT_FOUND, f"基金 {fund_code} 未找到", {"fund_code": fund_code})


def fund_data_fetch_failed(fund_code: str, cause: Exception = None) -> FundServiceError:
    return FundServiceError(
        ErrorCode.FUND_DATA_FETCH_FAILED, f"基金 {fund_code} 数据获取失败", {"fund_code": fund_code}, cause
    )


def market_data_fetch_failed(cause: Exception = None) -> MarketServiceError:
    return MarketServiceError(ErrorCode.MARKET_DATA_FETCH_FAILED, "市场数据获取失败", cause=cause)


def cache_operation_failed(operation: str, key: str, cause: Exception = None) -> CacheServiceError:
    return CacheServiceError(
        ErrorCode.CACHE_OPERATION_FAILED, f"缓存操作失败: {operation}", {"operation": operation, "key": key}, cause
    )
