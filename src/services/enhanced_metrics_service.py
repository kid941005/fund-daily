...
class EnhancedMetricsService(MetricsService):
    ...
    def _check_error_rate_alert(self, alert_config: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        with self._lock:
            # 获取窗口内的请求
            recent_requests = [
                r for r in self._history["requests"]
    ...
    def _check_cache_miss_alert(self, alert_config: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        with self._lock:
            # 获取窗口内的缓存操作
            recent_cache_ops = [
                c for c in self._history["cache"]
                if current_time - c["timestamp"] <= window_seconds
            ]
    ...
    def get_enhanced_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_cache_ops = [
                c for c in self._history["cache"]
                if current_time - c["timestamp"] <= recent_window
            ]
        ...
        enhanced_metrics = {
            **base_metrics,
            ...
            "alerts": {
                ...
                "config": {name: {"enabled": config["enabled"], "threshold": config["threshold"]}
...
def get_enhanced_metrics_service(redis_client=None) -> EnhancedMetricsService:
    ...
    if _enhanced_metrics_instance is None:
        _enhanced_metrics_instance = EnhancedMetricsService(
