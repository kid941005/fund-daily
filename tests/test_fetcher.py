...
class TestFetchFundData:
    ...
    def test_fetch_fund_data_success(self, mock_request):
        ...
        mock_request.return_value = 'jsonpgz({"fundcode":"000001","name":"测试基金","gszzl":"1.5"});'
        
        result = fetch_fund_data("000001")
        
        assert result["fundcode"] == "000001"
        assert result["name"] == "测试基金"
        assert result["gszzl"] == "1.5"
    ...
    def test_fetch_uses_cache(self, mock_request):
        ...
        mock_request.return_value = 'jsonpgz({"fundcode":"000001","name":"测试","gszzl":"1.0"});'
...
class TestFetchHotSectors:
    ...
    def test_fetch_hot_sectors_success(self, mock_request):
        ...
        mock_request.return_value = '''
        {
            "data": {
                "diff": [
                    {"f14": "新能源", "f3": 2.5, "f12": "8801"},
                    {"f14": "医药", "f3": -1.2, "f12": "8802"}
                ]
            }
        }
        '''
