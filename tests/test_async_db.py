...
class TestAsyncDatabase:
    ...
    async def test_execute_update(self, async_db):
        ...
        await async_db.execute(
            "INSERT INTO funds (fund_code, fund_name) VALUES ($1, $2) ON CONFLICT DO NOTHING", "TEST001", "测试基金"
        )

        # 更新
        count = await async_db.execute_update(
            "UPDATE funds SET fund_name = $1 WHERE fund_code = $2", "测试基金更新", "TEST001"
        )
    ...
    async def test_transaction(self, async_db):
        ...
        async with async_db.transaction() as conn:
            await conn.execute("INSERT INTO funds (fund_code, fund_name) VALUES ($1, $2)", "TEST_TXN", "事务测试基金")

        # 验证插入成功
        result = await async_db.fetchrow("SELECT fund_name FROM funds WHERE fund_code = $1", "TEST_TXN")
    ...
    async def test_transaction_rollback(self, async_db):
        ...
        try:
            async with async_db.transaction() as conn:
                await conn.execute(
                    "INSERT INTO funds (fund_code, fund_name) VALUES ($1, $2)", "TEST_ROLLBACK", "回滚测试"
                )
        ...
        # 验证插入被回滚
        result = await async_db.fetchrow("SELECT fund_name FROM funds WHERE fund_code = $1", "TEST_ROLLBACK")
...
class TestAsyncDBPerformance:
    ...
    @pytest.mark.skipif(os.getenv("SKIP_PERF_TESTS") == "1", reason="Performance tests skipped")
