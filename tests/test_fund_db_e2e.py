...
class TestFundDataLifecycle:
    ...
    def test_fund_data_crud(self):
        """测试基金数据的增删改查"""
        from db import (
            save_fund_info, get_fund_info,
            save_fund_nav, get_fund_nav,
            save_fund_score, get_fund_score,
            save_fund_data
        )
    ...
    def test_historical_data(self):
        """测试历史数据存储和查询"""
        from db import save_fund_nav, get_fund_history
        ...
        def write_fund_data(thread_id):
            ...
            try:
                ...
                for i in range(3):
                    save_fund_info(
                        fund_code=fund_code,
                        fund_name=f"线程{thread_id}基金{i}",
                        fund_size=100.0 + thread_id * 10 + i
                    )
                    results.append((thread_id, i, 'success'))
