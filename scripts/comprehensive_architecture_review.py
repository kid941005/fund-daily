...
class ArchitectureReviewer:
    ...
    def analyze_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            return {"path": str(file_path.relative_to(self.project_root)), "error": str(e), "issues": []}
        ...
        for file_path, file_info in self.results["files"].items():
            for issue in file_info.get("issues", []):
                ...
                if category == "architecture":
                    self.results["architectural_issues"].append({"file": file_path, "issue": issue})
                elif category == "performance":
                    self.results["performance_issues"].append({"file": file_path, "issue": issue})
                elif category == "security":
                    self.results["security_issues"].append({"file": file_path, "issue": issue})
                elif category == "maintainability":
                    self.results["maintainability_issues"].append({"file": file_path, "issue": issue})
                else:
                    self.results["code_quality_issues"].append({"file": file_path, "issue": issue})

    def _generate_recommendations(self):
        """生成优化建议"""
        total_issues = sum(self.results["summary"]["issues_by_category"].values())

        self.results["recommendations"] = {"high_priority": [], "medium_priority": [], "low_priority": []}
        ...
        if self.results["summary"]["issues_by_category"]["architecture"] > 0:
            self.results["recommendations"]["high_priority"].append("进行架构重构，解决耦合问题和设计缺陷")

        # 性能建议
        if self.results["summary"]["issues_by_category"]["performance"] > 0:
            self.results["recommendations"]["medium_priority"].append("优化性能热点，减少冗余计算和资源占用")

        # 可维护性建议
        if self.results["summary"]["issues_by_category"]["maintainability"] > 0:
            self.results["recommendations"]["low_priority"].append("改善代码注释和文档，提高可维护性")
