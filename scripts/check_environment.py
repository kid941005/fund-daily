...
class EnvironmentChecker:
    ...
    def check(self, name: str, condition: bool, message: str, critical: bool = False):
        ...
        self.results.append(
            {"name": name, "status": status, "condition": condition, "message": message, "critical": critical}
        )
    ...
    def check_python_version(self) -> bool:
        ...
        return self.check(
            "Python版本", major == 3 and minor >= 11, f"当前版本: {version} (需要 >= 3.11)", critical=True
        )
    ...
    def check_required_modules(self) -> bool:
        ...
        for module, description in required_modules:
            try:
                __import__(module)
                self.check(f"模块: {module}", True, f"{description} - 已安装")
            except ImportError:
                all_passed = self.check(f"模块: {module}", False, f"{description} - 未安装", critical=True)
        ...
        for module, description in optional_modules:
            try:
                __import__(module)
                self.check(f"模块: {module} (可选)", True, f"{description} - 已安装")
            except ImportError:
                self.check(f"模块: {module} (可选)", False, f"{description} - 未安装，某些功能受限")
    ...
    def check_environment_variables(self) -> bool:
        ...
        for var, description, required in env_vars:
            ...
            if required and not value:
                all_passed = self.check(f"环境变量: {var}", False, f"{description} - 未设置（必需）", critical=True)
            elif value:
                # 敏感信息隐藏
                display_value = value
                ...
                self.check(f"环境变量: {var}", True, f"{description} - 已设置 ({display_value})")
            else:
                self.check(f"环境变量: {var}", False, f"{description} - 未设置")
    ...
    def check_database_connection(self) -> bool:
        ...
        try:
            ...
            if not password:
                return self.check("数据库连接", False, "数据库密码未设置，无法测试连接", critical=True)
            ...
            try:
                ...
                return self.check("数据库连接", True, f"成功连接到 {host}")
            except Exception as e:
                return self.check("数据库连接", False, f"连接失败: {str(e)}", critical=True)

        except ImportError:
            return self.check("数据库连接", False, "psycopg2模块未安装，无法测试连接", critical=True)
    ...
    def check_redis_connection(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if r.ping():
                    return self.check("Redis连接", True, f"成功连接到 {host}:{port}")
                else:
                    return self.check("Redis连接", False, f"连接到 {host}:{port} 但ping失败")
            except Exception as e:
                return self.check("Redis连接", False, f"连接失败: {str(e)}")

        except ImportError:
            return self.check("Redis连接", False, "redis模块未安装，Redis功能不可用")
    ...
    def check_config_file(self) -> bool:
        ...
        if os.path.exists(config_path):
            ...
            try:
                ...
                return self.check("配置文件", True, f"配置文件存在，包含 {len(config)} 个配置项")
            except Exception as e:
                return self.check("配置文件", False, f"配置文件损坏: {str(e)}", critical=True)
        else:
            return self.check("配置文件", False, "配置文件不存在，将使用环境变量和默认值")
        ...
        for name, path in directories:
            if os.path.exists(path):
                ...
                if os.access(path, os.W_OK):
                    self.check(f"目录权限: {name}", True, f"{path} - 可写")
                else:
                    all_passed = self.check(f"目录权限: {name}", False, f"{path} - 不可写", critical=True)
            else:
                # 检查父目录权限
                parent_dir = os.path.dirname(path)
                if os.path.exists(parent_dir) and os.access(parent_dir, os.W_OK):
                    self.check(f"目录权限: {name}", True, f"{path} - 目录不存在但可以创建")
                else:
                    all_passed = self.check(
                        f"目录权限: {name}", False, f"{path} - 目录不存在且父目录不可写", critical=True
                    )
    ...
    def check_security(self) -> bool:
        ...
        if secret_key:
            if len(secret_key) >= 32:
                self.check("密钥强度", True, "Flask密钥长度足够（>=32字符）")
            else:
                all_passed = self.check(
                    "密钥强度", False, f"Flask密钥长度不足: {len(secret_key)}字符（需要>=32）", critical=True
                )
        else:
            self.check("密钥强度", False, "Flask密钥未设置，将使用临时密钥（仅适用于开发）")

        # 检查JWT密钥
        jwt_secret = os.getenv("FUND_DAILY_JWT_SECRET")
        if jwt_secret:
            if len(jwt_secret) >= 32:
                self.check("JWT密钥强度", True, "JWT密钥长度足够（>=32字符）")
            else:
                all_passed = self.check(
                    "JWT密钥强度", False, f"JWT密钥长度不足: {len(jwt_secret)}字符（需要>=32）", critical=True
                )
