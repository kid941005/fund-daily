...
def generate_migration_patch(file_path, matches):
    ...
    for match in matches:
        ...
        if "from src.config import get_config" not in "".join(lines[:line_num]):
            patches.append({"type": "import", "line": 0, "content": "from src.config import get_config\n"})
        ...
        patches.append({"type": "replace", "line": line_num, "old": old_line, "new": new_line.rstrip()})
...
def main():
    ...
    for file_path in python_files:
        ...
        if matches:
            all_matches.append({"file": file_path, "matches": matches})
