...
def migrate_market_service():
    ...
    init_pattern = r'(def __init__\(self, cache_enabled: bool = True\):.*?self\.metrics_service = get_metrics_service\(\))'
    ...
    content = re.sub(r'redis_set\((\w+),\s*(\w+),\s*ttl=(\w+)\)', 
                    r'self.cache_manager.set(\1, \2, ttl=\3)', content)
    
    # 替换没有ttl参数的redis_set
    content = re.sub(r'redis_set\((\w+),\s*(\w+)\)', 
                    r'self.cache_manager.set(\1, \2)', content)
