...
def identify_key_functions():
    ...
    fetcher_files = [
        ...
        ("src/fetcher/enhanced_fetcher.py", ["fetch_enhanced_fund_data", "fetch_fund_metrics"]),
        ("src/fetcher/xueqiu.py", ["fetch_xueqiu_data"]),
        ("src/fetcher/alipay.py", ["fetch_alipay_fund_data"]),
    ]
    ...
    service_files = [
        (
            ...
            ["get_market_sentiment", "get_commodity_sentiment", "get_hot_sectors", "get_market_news"],
        ),
        (
            "src/services/quant_service.py",
            ["calculate_dynamic_weights", "get_timing_signals", "portfolio_optimization", "rebalancing_suggestions"],
        ),
    ]

    # 3. 数据库模块 - 数据操作密集型
    db_files = [
        ("db/users.py", ["create_user", "get_user_by_id", "update_user"]),
        ("db/holdings.py", ["get_holdings", "add_holding", "update_holding", "delete_holding"]),
    ]
