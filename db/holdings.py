...
def get_holdings(user_id):
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            cursor.execute("SELECT code, name, amount, buy_nav, buy_date FROM holdings WHERE user_id = %s", (user_id,))
...
def add_to_watchlist(user_id, code):
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            cursor.execute(
                "INSERT INTO watchlist (user_id, code) VALUES (%s, %s) ON CONFLICT DO NOTHING", (user_id, code)
            )
            conn.commit()
