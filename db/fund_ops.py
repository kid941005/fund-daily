...
def save_fund_info(
    ...
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            ...
            """,
                (fund_code, fund_name, fund_type, fund_company, establish_date, fund_size, manager, risk_level, rating),
            )
            conn.commit()
...
def save_fund_nav(
    ...
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            # 首先确保基金基本信息存在
            cursor.execute(
                "INSERT INTO funds (fund_code, fund_name) VALUES (%s, %s) ON CONFLICT DO NOTHING",
...
def get_fund_nav(fund_code, nav_date=None):
    ...
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            if nav_date:
                cursor.execute("SELECT * FROM fund_nav WHERE fund_code = %s AND nav_date = %s", (fund_code, nav_date))
            else:
                cursor.execute(
                    "SELECT * FROM fund_nav WHERE fund_code = %s ORDER BY nav_date DESC LIMIT 1", (fund_code,)
                )
            row = cursor.fetchone()
            return dict(row) if row else None
...
def get_fund_score(fund_code, score_date=None):
    ...
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            if score_date:
                cursor.execute(
                    "SELECT * FROM fund_scores WHERE fund_code = %s AND score_date = %s", (fund_code, score_date)
                )
            else:
                cursor.execute(
                    "SELECT * FROM fund_scores WHERE fund_code = %s ORDER BY score_date DESC LIMIT 1", (fund_code,)
                )
            row = cursor.fetchone()
            return dict(row) if row else None
...
def get_fund_history(fund_code, days=30):
    ...
    with get_db() as conn:
        with get_cursor(conn) as cursor:
            ...
            return {"fund_info": fund_info, "nav_history": nav_history, "score_history": score_history}
