...
def get_holdings(user_id):
    ...
    conn = get_db()
    cursor = conn.execute(
        "SELECT code, name, amount, buy_nav, buy_date FROM holdings WHERE user_id = ? AND amount > 0",
        (user_id,)
    )
...
def delete_holding(user_id, code):
    ...
    conn = get_db()
    conn.execute(
        "DELETE FROM holdings WHERE user_id = ? AND code = ?",
        (user_id, code)
    )
    conn.commit()
    conn.close()
...
def get_user_config(user_id):
    ...
    conn = get_db()
    cursor = conn.execute(
        "SELECT config_json FROM config WHERE user_id = ?",
        (user_id,)
    )
    row = cursor.fetchone()
    conn.close()
...
def add_to_watchlist(user_id, code):
    ...
    try:
        conn.execute(
            "INSERT INTO watchlist (user_id, code) VALUES (?, ?)",
            (user_id, code)
        )
        conn.commit()
    except sqlite3.IntegrityError:
