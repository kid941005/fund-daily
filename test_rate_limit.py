...
def test_auth_rate_limit():
    ...
    # 测试注册接口（应该被限制）
    test_data = {"username": "testuser_" + str(int(time.time())), "password": "testpassword123"}
    ...
    for i in range(15):  # 超过10次/分钟的限制
        ...
        responses.append({"attempt": i + 1, "status": response.status_code, "headers": dict(response.headers)})
        time.sleep(0.1)  # 稍微延迟
...
def test_different_endpoints():
    ...
    for endpoint, name in endpoints:
        ...
        for i in range(25):  # 25次请求
            ...
            responses.append({"attempt": i + 1, "status": response.status_code})
            time.sleep(0.03)
